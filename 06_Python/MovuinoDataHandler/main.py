...
folderPath = "/Users/phelippeau/Documents/PhD/Data/Manip_Nohain/"

fileName = "record_5"  # generic name numbers will be added for duplicates

serialPort = '/dev/cu.usbserial-0154D98C'
...
if toDataManage:
    ...
    for i in range(file_start, file_start+nbRecord+1):
        ...
        if toVisualize:
            for i in range(file_start, file_start + nbRecord + 1):
                if (device == 'sensitivePen'):
                    sp.SensitivePenDataSet.PlotCompleteFile(folderPath + fileName + "_" + str(i) + "_treated_" + device,
                                                            sep, decimal)
                elif (device == 'globalDataSet'):
